<script lang="ts" module>
	...
	const { Story } = defineMeta({
		title: 'STARWAKE/buy_draco',
	});
</script>

<script lang="ts">
	/**
	 * buy_draco (240x) -- the reliable tier. 4 cells, 10 spins, ~84% completion.
	...
	import books from './data/buy_draco_books';
	import scenarios from './data/buy_draco_scenarios';
	...
	const play = async (index: number, label: string) => {
		...
		if (!data) {
			console.warn(`[${label}] no book at index ${index} -- re-export buy_draco`);
			return;
		}
...
<Story
	...
	args={templateArgs({
		skipLoadingScreen: true,
		data: {},
		action: async () => await play(at('draco_top_rung_2'), 'draco_top_rung_2'),
	})}
	{template}
/>
...
<Story
	...
	args={templateArgs({
		skipLoadingScreen: true,
		data: {},
		action: async () => await play(at('draco_roam_floor'), 'draco_roam_floor'),
	})}
	{template}
/>
